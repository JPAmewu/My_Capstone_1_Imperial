"""Build the focused Function 02 notebook from the Week 1 baseline contract."""
...
def build(repository_root: Path) -> Path:
    ...
    notebook.cells = [
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"overview\"></a>\n## 1. Overview\n\nThis focused notebook mirrors the authoritative Week 1 main notebook for Function 02. It analyses the ten immutable starter observations as a model-free random-search baseline."
        ),
        nbf.v4.new_markdown_cell(
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"environment-setup\"></a>\n## 4. Environment and setup\n\nLocate the repository and import the stable Function 02 analysis interface."
        ),
        nbf.v4.new_code_cell(
            "from pathlib import Path\nimport sys\nimport numpy as np\nimport pandas as pd\nimport matplotlib.pyplot as plt\n\n"
            "ROOT = Path.cwd().resolve()\nfor candidate in (ROOT, *ROOT.parents):\n"
            "    if (candidate / 'Week_01' / 'Function_02').is_dir(): ROOT = candidate; break\n"
            "else: raise FileNotFoundError('Could not locate the repository root.')\n"
            "CODE_DIR = ROOT / 'Week_01/Function_02/02_Code'\n"
            "if str(CODE_DIR) not in sys.path: sys.path.insert(0, str(CODE_DIR))\n"
            "from analyse_function_02 import analyse_function_02"
        ),
        nbf.v4.new_markdown_cell(
            "<a id=\"data-validation\"></a>\n## 5. Data validation\n\nLoad the ten starter rows and validate alignment, dimensionality, finiteness, and unit-square bounds."
        ),
        nbf.v4.new_code_cell(
            "observations, summary, diagnostic_figure = analyse_function_02(ROOT)\n"
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"descriptive-eda\"></a>\n## 6. Descriptive EDA\n\nFunction 02 is assessed only in its own objective scale. The maximum is an observed incumbent, not a global optimum."
        ),
        nbf.v4.new_code_cell("display(observations)\npd.Series(summary, name='Function 02 baseline')"),
        nbf.v4.new_markdown_cell(
            "<a id=\"visual-eda\"></a>\n## 7. Visual EDA\n\nShow the observation trace, running best, and both input/output relationships."
        ),
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"reproducibility-checks\"></a>\n## 10. Reproducibility checks\n\nConfirm the starter count, best row, and exact agreement with the Function 02 summary."
        ),
        nbf.v4.new_code_cell(
            "best_index = int(np.argmax(outputs))\n"
            "assert summary['observations'] == 10 and summary['dimensions'] == 2\n"
            "assert summary['best_query_number'] == best_index + 1 == 10\n"
            "assert np.isclose(summary['best_output'], outputs[best_index])\n"
            "assert np.allclose(summary['best_input'], inputs[best_index])\n"
            "print('Function 02 Week 1 baseline checks passed.')"
        ),
        nbf.v4.new_markdown_cell(
            "<a id=\"conclusions-next-steps\"></a>\n## 11. Conclusions and next steps\n\nThe ten starter observations establish a reproducible random-search baseline. Query 10 is the observed incumbent at `0.6112052`; later returned evidence belongs to the sequential ledger and later checkpoints."
        ),
    ]
    output = repository_root / "Week_01/Function_02/01_Notebook/Week_01_Function_02.ipynb"
