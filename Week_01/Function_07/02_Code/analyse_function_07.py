"""Reproduce the Week 1 Function 7 baseline summary and diagnostic figure."""
...
def analyse_function_07(repository_root: str | Path | None = None):
    """Return validated Function 7 starter observations, summary, and figure."""
    root = find_repository_root(repository_root)
    inputs, outputs = load_starter_data(7, repository_root=root)
    summary = observation_summary(inputs, outputs)
    summary.update({"function": "F7", "week": 1, "strategy": "Bayesian Optimisation"})
    figure = plot_function_diagnostics(
        inputs, outputs, title="Week 1 Function 7 — Bayesian Optimisation"
    )
...
def write_artifacts(
    ...
    root = find_repository_root(repository_root)
    function_root = root / "Week_01" / "Function_07"
    results, summary, figure = analyse_function_07(root)
    ...
    if write_figure:
        figure_path = figures_dir / "function_07_diagnostics.png"
        compatibility_path = figures_dir / "week_01_function_07_diagnostics.png"
...
def main() -> None:
    ...
    frame, summary, figure = analyse_function_07()
