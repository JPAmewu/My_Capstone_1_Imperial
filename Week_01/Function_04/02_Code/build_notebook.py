"""Build the focused Function 04 notebook from the Week 1 baseline contract."""
...
def build(repository_root: Path) -> Path:
    ...
    notebook.cells = [
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"overview\"></a>\n## 1. Overview\n\nThis focused notebook mirrors the authoritative Week 1 main notebook for Function 04. It analyses the 30 immutable starter observations as a model-free Grid Search baseline."
        ),
        nbf.v4.new_markdown_cell(
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"descriptive-eda\"></a>\n## 6. Descriptive EDA\n\nFunction 04 is assessed only in its own objective scale. Its negative maximum is valid: the least-negative observed value is the incumbent, not a global optimum."
        ),
        nbf.v4.new_code_cell("display(observations)\npd.Series(summary, name='Function 04 baseline')"),
        nbf.v4.new_markdown_cell(
        ...
        nbf.v4.new_markdown_cell(
            "<a id=\"gp-ucb-values\"></a>\n## 9. GP-UCB values\n\nNo GP-UCB proposal is claimed for the Week 1 baseline. The 30 starter values remain the authoritative evidence for this analysis."
        ),
        nbf.v4.new_markdown_cell(
            "<a id=\"reproducibility-checks\"></a>\n## 10. Reproducibility checks\n\nConfirm the starter count, best row, and exact agreement with the Function 04 summary."
        ),
        nbf.v4.new_code_cell(
            "best_index = int(np.argmax(outputs))\n"
            "assert summary['observations'] == 30 and summary['dimensions'] == 4\n"
            "assert summary['best_query_number'] == best_index + 1 == 28\n"
            "assert np.isclose(summary['best_output'], outputs[best_index])\n"
            "assert np.allclose(summary['best_input'], inputs[best_index])\n"
            "print('Function 04 Week 1 baseline checks passed.')"
        ),
        nbf.v4.new_markdown_cell(
            "<a id=\"conclusions-next-steps\"></a>\n## 11. Conclusions and next steps\n\nThe 30 starter observations establish a reproducible Grid Search baseline. Query 28 is the observed incumbent at `-4.02554228191`; later returned evidence belongs to the sequential ledger and later checkpoints."
        ),
    ]
    output = repository_root / "Week_01/Function_04/01_Notebook/Week_01_Function_04.ipynb"
