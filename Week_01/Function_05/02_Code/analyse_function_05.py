"""Reproduce the Week 1 Function 5 baseline summary and diagnostic figure."""
...
def analyse_function_05(repository_root: str | Path | None = None):
    """Return validated Function 5 starter observations, summary, and figure."""
    root = find_repository_root(repository_root)
    inputs, outputs = load_starter_data(5, repository_root=root)
    summary = observation_summary(inputs, outputs)
    summary.update({"function": "F5", "week": 1, "strategy": "Manual Reasoning"})
    figure = plot_function_diagnostics(
        inputs, outputs, title="Week 1 Function 5 — Manual Reasoning"
    )
...
def write_artifacts(
    ...
    root = find_repository_root(repository_root)
    function_root = root / "Week_01" / "Function_05"
    results, summary, figure = analyse_function_05(root)
    ...
    if write_figure:
        figure_path = figures_dir / "function_05_diagnostics.png"
        compatibility_path = figures_dir / "week_01_function_05_diagnostics.png"
...
def main() -> None:
    ...
    frame, summary, figure = analyse_function_05()
