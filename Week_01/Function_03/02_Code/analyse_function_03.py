"""Independent analysis for Week 01 Function 03."""
...
SUBMITTED_QUERY=np.array([0.444444,0.666666,0.333333],dtype=float)
SUBMITTED_RETURN=-0.04090761844901528

def analyse_function_03(data_dir: Path=ROOT/"03_Data") -> tuple[pd.DataFrame,dict]:
    ...
    summary={"function":"03","strategy":STRATEGY,"n_observations":int(len(y)),
...
def create_figure(table: pd.DataFrame,summary: dict) -> plt.Figure:
    ...
    fig.colorbar(im,ax=axes[1],label="Coordinate value"); fig.suptitle(f"Week 01 Function 03 — {STRATEGY}")
...
def write_artifacts(results_dir: Path=ROOT/"04_Results",figures_dir: Path=ROOT/"05_Figures") -> dict:
    ...
    table,summary=analyse_function_03(); results_dir.mkdir(parents=True,exist_ok=True); figures_dir.mkdir(parents=True,exist_ok=True)
    table.to_csv(results_dir/"observations.csv",index=False)
    (results_dir/"summary.json").write_text(json.dumps(summary,indent=2)+"\n",encoding="utf-8")
    fig=create_figure(table,summary); fig.savefig(figures_dir/"function_03_diagnostics.png",dpi=160,bbox_inches="tight"); plt.close(fig)
    return summary
...
def main() -> None:
    ...
    table,summary=analyse_function_03(); summary=write_artifacts() if args.write_artifacts else summary
