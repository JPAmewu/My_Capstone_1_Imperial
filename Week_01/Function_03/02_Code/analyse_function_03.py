"""Reproduce the Week 1 Function 3 baseline summary and diagnostic figure."""
...
def analyse_function_03(repository_root: str | Path | None = None):
    """Return validated Function 3 starter observations, summary, and figure."""
    root = find_repository_root(repository_root)
    inputs, outputs = load_starter_data(3, repository_root=root)
    summary = observation_summary(inputs, outputs)
    summary.update({"function": "F3", "week": 1, "strategy": "Grid Search"})
    figure = plot_function_diagnostics(
        inputs, outputs, title="Week 1 Function 3 — Grid Search"
    )
...
def write_artifacts(
    ...
    root = find_repository_root(repository_root)
    function_root = root / "Week_01" / "Function_03"
    results, summary, figure = analyse_function_03(root)
    ...
    if write_figure:
        figure_path = figures_dir / "function_03_diagnostics.png"
        compatibility_path = figures_dir / "week_01_function_03_diagnostics.png"
...
def main() -> None:
    ...
    frame, summary, figure = analyse_function_03()
