...
The Week 13 consolidated notebook executed all 96 code cells without error and applies the same eleven-stage analysis to Functions 1–8. It generates one valid, non-duplicate, correctly dimensioned UCB query per function. The earlier UCB/EI/PI sensitivity comparison remains in the Week 13 results CSV. GP optimisation may place some kernel parameters at configured bounds; fitted kernels are retained as diagnostics and should inform later sensitivity testing.
